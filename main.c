...
double parse_north(char *);
double parse_west(char *);
void display_lap(int);
...
double north_start = 0, west_start = 0;
...
void __interrupt (high_priority)ISR(void)
{
    if(INTCONbits.INT0IF)
    {
        ...
        north_start = parse_north(nmea_buffer);
        west_start = parse_west(nmea_buffer);
        display(1234);
        return;
    }
...
void main(void)
{
 //   LATEbits.LATE0 = 1;     //Colon
    int fix = 0, ready = 0;
    double gps_north = 0, gps_west = 0;
    ...
    while(1)
    {
        ...
        if(ready)
            fix = 1;
        gps_north = parse_north(nmea_buffer);
        gps_west = parse_west(nmea_buffer);
        if(gps_north == north_start && gps_west == west_start)
        {
            display_lap(seconds);
            seconds = 0;
        }
